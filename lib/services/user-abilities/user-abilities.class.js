  ...
  async find(params) {
    ...
    if(params.query.includeSchema){
      ...
      if(getValidators){
        ...
        // Add default fields
        jsonSchema.properties.createdAt = {format: 'date-time', meta: [], type: 'string', 'readOnly': true};
        jsonSchema.properties.updatedAt = {format: 'date-time', meta: [], type: 'string', 'readOnly': true};
        jsonSchema.properties._id = {meta: [], pattern: '^[0-9a-fA-F]{24}$', type: 'string', 'readOnly': true};
        if(jsonSchema){
