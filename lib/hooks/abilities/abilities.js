...
const abilities = async function(hook, name, method, testMode, userIdForTest, getRules = _getRules, getUser = _getUser) {
  try {
    ...
    if(testMode){
      hook.app.info('feathers-mongoose-casl - abilities hook - start abilities process for testMode', {action, serviceName});
    }else{
      hook.app.info('feathers-mongoose-casl - abilities hook - start abilities process', {action, serviceName});
    }
    

    // --------- Find user ------------
    // find user from Cache/DB
    hook.app.info('feathers-mongoose-casl - abilities hook - get user start');
    const {user, hasUser} = await getUser({hook, testMode,userIdForTest});
    hook.app.info('feathers-mongoose-casl - abilities hook - get user end', {hasUser, userId: hasUser ? user._id : null});
    // --------------------------------
    
    
    // --------- Find Rules ------------
    // find rules from Cache/DB
    hook.app.info('feathers-mongoose-casl - abilities hook - get rules start');
    const {rulesFromDb, defaultRules} = await getRules({hook, testMode});
    hook.app.info('feathers-mongoose-casl - abilities hook - get rule end', {rulesFromDb: rulesFromDb ? rulesFromDb.length : 0, defaultRules: defaultRules ? defaultRules.length : 0});
    // --------------------------------
    ...
    const ability = defineAbilities({hook, hasUser, user, defaultRules, rulesFromDb, testMode});
    hook.app.info('feathers-mongoose-casl - abilities hook - defineAbilities end', {action, serviceName});
    // --------------------------------
    ...
    const throwUnlessCan = (action, resource) => {
      if (ability.cannot(action, resource)) {
        hook.app.info('feathers-mongoose-casl - abilities hook - user not allowed', {action, serviceName});
    ...
    if(testMode) {
      ...
      hook.app.info('feathers-mongoose-casl - abilities hook - user allowed - test mode', {action, serviceName});
      return hook;
    }
    // --------------------------------
    ...
    if (!hook.id) {
      ...
      hook.app.info('feathers-mongoose-casl - abilities hook - user allowed', {action, serviceName});
      return hook;
    }
    // --------------------------------
