    ...
    roles.forEach(role => {
      if(role.type === 'blocked'){
        if(hasUser && isEqual(role.user, userId)){
          if(role.blockAll){
            hook.app.info(`feathers-mongoose-casl , src/hooks/abilities.js - block ${userId} user by ${role._id} ${role.name} role`);
            if(testMode) {
        ...
        if(hasUser &&
          ...
          if(!blockedRoles.length || !blockedRoles.includes(role._id)){
            ...
            hook.app.info(`feathers-mongoose-casl , src/hooks/abilities.js - block ${userId} from role ${role._id}`);
          }
        }
