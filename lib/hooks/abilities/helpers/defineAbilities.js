...
function defineAbilities({hook, hasUser, user, rulesFromDb, defaultRules, rulesFromConfig, rulesFromServiceOptions, testMode}) {
  ...
  const handleRule = function(rule){
    const {actions, subject, conditions, fields, userContext, roles, anonymousUser, whitelistPopulate} = rule;
    ...
    // we want add whitelistPopulate to sanitizedData.whitelistPopulate, we handle this before request
    if(['find', 'get'].includes(hook.method) && whitelistPopulate && whitelistPopulate.length){
      hook.app.debug('feathers-mongoose-casl sanitizedData update whitelistPopulate');
      const currentWhitelistPopulate = hook.params[defineAbilitiesHelpers.WHITELIST_POPULATE_KEY] || [];
      hook.params[defineAbilitiesHelpers.POPULATE_WHITELIST_KEY] = [...currentWhitelistPopulate, ...whitelistPopulate];
    }
