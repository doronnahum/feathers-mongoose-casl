...
const abilities = async function(hook, name, method, testMode, userIdForTest ) {
  try {
    ...
    if(testMode){
      hook.app.info('feathers-mongoose-casl , start abilities process for testMode', {action, serviceName});
    }else{
      hook.app.info('feathers-mongoose-casl , start abilities process', {action, serviceName});
    }
    ...
    const throwUnlessCan = (action, resource) => {
      if (ability.cannot(action, resource)) {
        hook.app.info('feathers-mongoose-casl , user not allowed', {action, serviceName});
