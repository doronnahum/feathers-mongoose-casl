  ...
  return async function(hook) {
    try {
      ...
      if(skipServices.includes(serviceName)) {
        hook.app.info(`feathers-mongoose-casl - validateSchema hook - skip service ${serviceName}`);
        return hook;
      }
      ...
      if(getValidators){
        ...
        if(validator){
          ...
          hook.app.info(`feathers-mongoose-casl - validateSchema hook - end, validate successfully, service ${serviceName}`);
          if(check.error){
            hook.app.info(`feathers-mongoose-casl - validateSchema hook - end, validate failed, service ${serviceName}`);
            throw new NotAcceptable(check.error);
          }
        }
        return hook;
      }else{
        hook.app.info(`feathers-mongoose-casl - validateSchema hook - end, getValidators is not found, service ${serviceName}`);
      }
    } catch (error) {
