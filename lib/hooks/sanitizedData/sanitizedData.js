  ...
  return async function(hook) {
    try {
      if(skipServices && skipServices.includes(hook.path)) {
        hook.app.info(`feathers-mongoose-casl - sanitizedData hook - skip - path: ${hook.path}`);
        return;
      }
      ...
      if(!abilityFields || !abilityFields.length) {
        hook.app.info(`feathers-mongoose-casl - sanitizedData hook - end, the abilityFields in not found - path: ${hook.path}`);
        return hook;
      }
      if(!hook.params[defineAbilitiesHelpers.ENABLE_ABILITY_FIELDS_KEY]) {
        hook.app.info('feathers-mongoose-casl - sanitizedData hook - end');
        return hook;
      };
      
      if(hookType === 'after' && action === 'find'){
        if(hook.result.data){
          hook.app.info(`feathers-mongoose-casl - sanitizedData hook - filter data after find, service ${hook.path}`);
          hook.result.data = hook.result.data.map(doc => pick(doc, abilityFields));
        }
        return hook;
      }
      if(hookType === 'after' && action === 'get'){
        hook.app.info(`feathers-mongoose-casl - sanitizedData hook - filter data after get, service ${hook.path}`);
        ...
        if(populate){
          ...
          populate = populateWhitelist && populateArr.filter(item => populateWhitelist.includes(item)).join(' ');
          if(populate && populate.trim().length){
            hook.app.info('feathers-mongoose-casl - sanitizedData hook - protect populate', { before:hook.params.query['$populate'], after: populate});
            hook.params.query['$populate'] = populate;
          }else{
            hook.app.info('feathers-mongoose-casl - sanitizedData hook - protect populate - remove populate from request');
        ...
        if(_isSimpleAbilityFields){ // we can use mongoose to protect fields
          ...
          hook.app.info(`feathers-mongoose-casl - sanitizedData hook - before ${action}, protect fields with feathers select, service ${hook.path}, select: ${hook.params.query['$select']}`);
      ...
      if(hookType === 'before' && ['create', 'update', 'patch'].includes(action)){
        if(hook.data && typeof hook.data === 'object'){
          ...
          hook.app.info(`feathers-mongoose-casl - sanitizedData hook - protect fields before ${action}`);
