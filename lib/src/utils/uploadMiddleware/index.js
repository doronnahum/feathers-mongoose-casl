...
const uploadMiddleware = function (config) {
  ...
  if (
    !hasOwnProperty(config, 'publicRead') &&
    (hasOwnProperty(config, 'public') || hasOwnProperty(config, 'protectTheFile'))
  ) {
    console.error('Feathers-mongoose-casl-uploadsHooks -please stop using protectTheFile&public , use publicRead instead');
    // @ts-ignore
    if (hasOwnProperty(config, 'public')) config.publicRead = config.public;
    // @ts-ignore
    if (hasOwnProperty(config, 'protectTheFile')) config.publicRead = !config.protectTheFile;
  }
  const {
    app,
    fileKeyName = 'file',
  ...
  */
  ...
  return async function (req, res, next) {
     ...
     */
      ...
      *
      */
      if (['PATCH', 'PUT'].includes(req.method) && req.file) {
        try {
