...
const handleUpload = async function (params) {
  const { app, req, fileKeyName, uploadService, public, storageService } = params;
  try {
    ...
    if (isFile) {
       ...
       */
      const upload = await uploadService.create({ fileId, buffer: req[fileKeyName], [UPLOAD_PUBLIC_FILE_KEY]: public });
