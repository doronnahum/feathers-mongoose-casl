...
const uploadServiceNameByStorageTypes = {
...
const getFileUrl = function (payload) {
  ...
  if (storageService === STORAGE_TYPES['local-private']) {
    return `${app.get('feathers-mongoose-casl').privateFolder}/uploads/${fileId}`;
  }
...
const uploadMiddleware = function (payload) {
  ...
  const multipartMiddleware = multer(multerConfig);
  /**
  ...
  return async function (req, res, next) {
    ...
    const uploadServiceName = uploadServiceNameByStorageTypes[storageService]; // for example - 's3' will be 'uploads-s3'
    if (!uploadServiceName) {
      throw new Error('feathers-mongoose-casl - uploadMiddleware - storageService need to be onOf ' + Object.keys(uploadServiceNameByStorageTypes).join(','));
    }
    ...
    if (req.method === 'DELETE') {
      try {
        ...
        if (currentDoc && currentDoc.storage !== enums.STORAGE_TYPES.others) {
          ...
          const currentFileServiceName = uploadServiceNameByStorageTypes[currentDoc.storage];
    ...
    if (['POST', 'PATCH', 'PUT'].includes(req.method)) {
      ...
      if (req[fileKeyName] && typeof req[fileKeyName] === 'object') { // If it a string then we didn't need to upload
        try {
          ...
          const upload = await app.service(uploadServiceName).create({ fileId, buffer: req[fileKeyName], [enums.uploadPublicFileKey]: publicAcl });
          if (upload && upload.id) {
