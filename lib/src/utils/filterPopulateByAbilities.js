...
/**
   ...
   * @example
   * const populateArray = ['users', {path: 'office', populate: 'organizations'}]
   * const populateWhitelist = ['users', 'office', 'office.banks']
   * const populate = filterPopulateByAbilities(populate, populateWhitelist) === ['users', {path: 'office'}];
   * @returns {array}
   */
const filterPopulateByAbilities = function (populate, populateWhitelist, prefix = '') {
  const populateArray = getPopulateAsArray(populate);
    ...
    /**
      ...
      if (populate) {
        ...
        /**
           ...
           */
        const allowDeepPopulate = _populate.length > 0;
