...
module.exports = function returnUserOnLogin () {
  return async function (hook) {
    if (hook.result && hook.result.accessToken) {
      ...
      if (!hook.params.user) {
        // In test mode ror some unknown reason the user is missing in the params
        const email = hook.data.email;
        const usersRes = await hook.app.service('users').find({ query: { email } });
        hook.params.user = usersRes.data[0];
      }
