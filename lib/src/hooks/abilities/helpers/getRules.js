...
const getRules = async function (payload) {
  ...
  try {
    ...
    const serviceRules = (service && service.options) && (service.options.abilities || service.options.serviceRules)
    if (!isProduction && serviceRules && service.options.serviceRules) {
      console.warn('Please rename serviceRules in the service option to abilities');
    }
    if (serviceRules) {
      rulesFromServiceOptions = serviceRules.map(rule => {
