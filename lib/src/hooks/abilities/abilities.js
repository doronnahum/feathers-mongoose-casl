...
const abilities = async function (
  ...
  try {
    ...
    let populate;
    // roles are depending on _id of doc, and populate doc can break this check
    // something like: conditions: { _id: { $in: '{{ user.organizations }}' } }
    // we need organizations ids and not all doc to verify this condition
    if (hook.params.query && hook.params.query.$populate) {
      populate = hook.params.query.$populate;
    ...
    if (action === 'get') {
      if (populate) { // server what user ask - we remove thie populate to verify abilites
        hook.params.query.$populate = populate;
        hook.result = await service.get(hook.id, params)
      } else {
        hook.result = result;
      }
    }
