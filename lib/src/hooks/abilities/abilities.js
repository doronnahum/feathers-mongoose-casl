  ...
  try {
    ...
    if(skipAbilitiesCheck){
      if(skipAbilitiesCheck === true || skipAbilitiesCheck.includes(action)) {
        debug('abilities hook - skip abilities by service.options.skipAbilitiesCheck', {action, name, options: service.options});
        return hook;
      }
    }
    const serviceName = name || hook.path;
    if(!serviceName){
      hook.app.error('feathers-mongoose-casl - abilities hook - missing serviceName && hook.path');
      return;
    }
    if(testMode){
      debug('abilities hook - start abilities process for testMode', {action, serviceName});
    }else{
      debug('abilities hook - start abilities process', {action, serviceName});
    }
    

    // --------- Find user ------------
    // find user from Cache/DB
    debug('abilities hook - get user start');
    
    const {user, hasUser} = await getUser({hook, allowUserFromCache: true, testMode,userIdForTest});
    debug('abilities hook - get user end', {hasUser, userId: hasUser ? user._id : null});
    // --------------------------------
    
    // --------- Find Rules ------------
    // find rules from Cache/DB
    debug('abilities hook - get rules start');
    const {rulesFromDb, defaultRules, rulesFromConfig, rulesFromServiceOptions} = await getRules({hook, serviceName, action });
    debug(
      'abilities hook - get rule end',
      {
    ...
    debug('abilities hook - defineAbilities end', {action, serviceName});
    // --------------------------------
    ...
    if(testMode) {
      ...
      debug('abilities hook - user allowed - test mode', {action, serviceName});
      return hook;
    }
    // --------------------------------
