...
const userCache = {
  ...
  initCache: function(hook){
    if(!userCache.usersCacheMap){
      const userCacheConfig = hook.app.get('feathers-mongoose-casl').usersCache;
      debug('usersCache hook - initial the users cache');
      ...
      if(hookType === 'after' && method === 'get'){
        ...
        if(userId && hook.result && hook.result._id){
          if(!hook.result.fromCache){
            ...
            debug('usersCache hook - save user in cache', {userId});
            return hook;
          }
          return hook;
        }
  ...
  clearUserFromCache: async function(hook, userId, message) {
    ...
    try {
      ...
      if(userId){
        ...
        hook.app.info(message || `feathers-mongoose-casl - usersCache hook - remove user from cache after ${method}`, {userId});
        return;
      }
  ...
  getUserFromCache: async function(hook, userId) {
    ...
    if(userFromCatch){
      debug('usersCache hook - return user from cache', {userId});
    }
