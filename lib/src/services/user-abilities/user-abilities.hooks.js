...
const pickFromArr = function toObject (arr = [], fields) {
  var rv = {};
  for (var i = 0; i < arr.length; ++i) { if (arr[i] !== undefined) rv[arr[i]] = arr[i]; }
...
module.exports = {
  ...
  after: {
    all: [sanitizedData],
    find: [validateSchema, async (hook) => {
      ...
      // Check read
      const readCheck = await abilities.testAbilities(hook, serviceName, 'read', userId);
      ...
      const createCheck = await abilities.testAbilities(hook, serviceName, 'create', userId);
      result.canCreate = isPassAbilityCheck(createCheck);
      result.createFields = result.canCreate ? getAbilityFields(createCheck) : null;

      const updateCheck = await abilities.testAbilities(hook, serviceName, 'update', userId);
      result.canUpdate = isPassAbilityCheck(updateCheck);
      result.updateFields = result.canCreate ? getAbilityFields(updateCheck) : null;

      const deleteCheck = await abilities.testAbilities(hook, serviceName, 'remove', userId);
