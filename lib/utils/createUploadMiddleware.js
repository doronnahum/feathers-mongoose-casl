...
module.exports = function createUploadMiddleware({app, fileKeyName = 'file', uploadToStatic = false, serviceName}) {
  return async  function(req,res,next){
    const upload_serviceName = uploadToStatic ? 'uploads-static' : 'uploads';
    ...
    if(req.method === 'DELETE') {
      try {
        ...
        if(file && file.storage !== enums.STORAGE_TYPES.others){
          ...
          await app.service(upload_serviceName).remove(file.fileId);
          next();
        }
      }
      catch (error) {
    ...
    if(['POST', 'PATCH', 'PUT'].includes(req.method)){
      ...
      if(['PATCH', 'PUT'].includes(req.method) && req[fileKeyName]){
        try {
          ...
          if(currentDoc && currentDoc.storage !== enums.STORAGE_TYPES.others){
            await app.service(upload_serviceName).remove(currentDoc.fileId);
          }
        } catch (error) {
          app.error(`feathers-mongoose-casl - try to remove ${req.params.__feathersId} from storage failed, method: ` + req.method);
        }
      ...
      if(req[fileKeyName] && typeof req[fileKeyName] === 'object'){ // If it a string then we didn't need to upload
        try {
          const upload = await app.service(upload_serviceName).create({buffer: req[fileKeyName]});
          ...
          if(upload && upload.id){
            ...
            // req.body[fileKeyName] = `${serverUrl}/get-file?s=${serviceName}&id=${req.body._id}`;
            next();
          }else{
